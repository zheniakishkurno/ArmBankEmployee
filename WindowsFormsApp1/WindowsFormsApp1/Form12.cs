...
namespace WindowsFormsApp1
{
    public partial class Form12 : Form
    {
        ...
        {
            string query = "SELECT * FROM reservation;"; // SQL-запрос для выборки всех данных из таблицы program
        ...
        private void guna2Button2_Click(object sender, EventArgs e)
        {
            ...
            if (guna2DataGridView1.SelectedRows.Count > 0)
            {
                ...
                foreach (DataGridViewRow row in guna2DataGridView1.SelectedRows)
                {
                    ...
                    {
                        int idreservation = Convert.ToInt32(row.Cells["idreservation"].Value); // Предположим, что "id" - это имя столбца с уникальным идентификатором строки
                        idsToDelete.Add(idreservation);
                    }
                }
        ...
        private void DeleteRowsFromDatabase(List<int> ids)
        {
            try
            {
                ...
                foreach (int idreservation in ids)
                {
                    string query = $"DELETE FROM reservation WHERE idreservation = {idreservation};"; // Предположим, что "program" - это имя вашей таблицы
                    MySqlCommand cmd = new MySqlCommand(query, connection);
                    cmd.ExecuteNonQuery();
                }
            }
