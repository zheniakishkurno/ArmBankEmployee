...
namespace WindowsFormsApp1
{
    public partial class Form15 : Form
    {
        ...
        public Form15(int background, int plauplay, string username)
        {
            InitializeComponent();
        ...
        {
            string query = "SELECT * FROM `order`;"; // SQL-запрос для выборки всех данных из таблицы order
        ...
        private void guna2Button2_Click_1(object sender, EventArgs e)
        {
            ...
            if (guna2DataGridView1.SelectedRows.Count > 0)
            {
                ...
                foreach (DataGridViewRow row in guna2DataGridView1.SelectedRows)
                {
                    ...
                    {
                        int idorder = Convert.ToInt32(row.Cells["idorder"].Value); // Предположим, что "id" - это имя столбца с уникальным идентификатором строки
                        idsToDelete.Add(idorder);
                    }
                }
        ...
        private void DeleteRowsFromDatabase(List<int> ids)
        {
            try
            {
                ...
                {
                    string query = $"DELETE FROM `order` WHERE idorder = {idorder};"; // Предположим, что "order" - это имя вашей таблицы
                    MySqlCommand cmd = new MySqlCommand(query, connection);
                    cmd.ExecuteNonQuery();
                }
            }
